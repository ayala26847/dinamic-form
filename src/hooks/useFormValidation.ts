...
} from "../services/validation/fieldValidation";
import { FieldErrors, FieldRules, FormData } from "../types/form/form.types";
...
export const useFormValidation = ({
  ...
  const setFieldValue = useCallback(
    (fieldLabel: string, value: string | number) => {
  ...
  const validateSingleField = useCallback(
    (fieldLabel: string, value: string | number, rules: FieldRules) => {
  ...
  const validateAllFields = useCallback(
    (fieldsRules: Record<string, FieldRules>): boolean => {
  ...
  const isFormValid =
    Object.keys(fieldErrors).every((key) => fieldErrors[key].length === 0) &&
    Object.keys(fieldErrors).length > 0;

  // Reset form to initial or new data
  const resetForm = useCallback(
    (newData: FormData = initialData) => {
      setFormData(newData);
      setFieldErrors({});
    },
    [initialData]
